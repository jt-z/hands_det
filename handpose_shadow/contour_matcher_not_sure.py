...
class ContourMatcher:
    ...
    def count_fingers_from_defects(self, contour, defects):
        """
        基于凸缺陷检测手指数量
        原理：手指之间的凹陷对应凸缺陷
        """
        ...
        for i in range(defects.shape[0]):
            ...
            # 角度阈值判断是否为手指间隙
            if angle <= np.pi/2 and d > 10000:  # 深度阈值
                finger_count += 1
        
        return min(finger_count, 5)  # 最多5根手指

    def geometric_feature_matching(self, contour1, contour2):
        """
        基于几何特征的匹配，最适合手影识别
        原理：提取手部的几何特征，如手指数量、凸包比例、主轴方向等
        """
        def extract_hand_features(contour):
            ...
            if len(hull_indices) > 3:
                ...
                finger_count = self.count_fingers_from_defects(contour, defects)
            else:
                finger_count = 0
        ...
        # 提取特征
        features1 = extract_hand_features(contour1)
        features2 = extract_hand_features(contour2)
        
        # 加权相似度计算
        weights = np.array([0.2, 0.2, 0.15, 0.15, 0.3])  # 手指数量权重最高
        
        # 归一化差异
        diff = np.abs(features1 - features2)
        normalized_diff = diff / (features1 + features2 + 1e-10)
        
        # 计算加权相似度
        weighted_diff = np.sum(weights * normalized_diff)
        similarity = 100 / (1 + weighted_diff * 20)
